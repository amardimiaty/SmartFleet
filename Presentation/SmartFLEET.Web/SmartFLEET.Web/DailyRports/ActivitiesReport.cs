...
namespace SmartFLEET.Web.DailyRports
{
    ...
    public class ActivitiesReport
    {
        ...
        /// <returns></returns>
        public List<TargetViewModel> BuildDailyReport(List<Position> positions, DateTime startPeriod,
            string vehicleName)
        {
        ...
        private static Periods GetLastPeriods(List<Periods> periods, IOrderedEnumerable<Position> orderedEnumerable)
        {
            ...
            var poriod = new Periods(enumerable.LastOrDefault()?.Timestamp, end,
                enumerable.LastOrDefault()?.MotionStatus);
            return poriod;
        }

        // ReSharper disable once MethodTooLong
        // ReSharper disable once TooManyArguments
        private static List<TargetViewModel> BuildReport(List<Position> positions, DateTime startPeriod,
            string vehicleName, List<Periods> periods)
        {
        ...
        // ReSharper disable once TooManyArguments
        private static bool MotionStatusChanged(Position position, List<Periods> periods,
            ref MotionStatus? currentStatus,
            ref DateTime? start)
        {
        ...
        // ReSharper disable once MethodTooLong
        // ReSharper disable once TooManyArguments
        private static void GetDistanceAndDuration(List<Position> positions, Position lastPosition,
            ...
            TargetViewModel trgt)
        {
            ...
            if (lastPosition != null && firstPosition != null)
            {
                ...
                trgt.Distance =
                    Math.Round(GeofenceHelper.HaversineFormula(p1, p2, GeofenceHelper.DistanceType.Kilometers), 2);
